...
namespace BetterPathfinding {
    public class NewPathFinder {
...
#endif
                            } else {    //Don't reopen closed nodes if the path cost difference isn't large enough to justify it; otherwise there can be cascades of revisiting the same nodes over and over for tiny path improvements each time
        ...
        private short GetTotalPerceivedPathCost(TraverseParms traverseParms, bool canPassAnything, bool shouldCollideWithPawns, Pawn pawn, PawnPathCostSettings pawnPathCosts) {
            ...
            if (pawnPathCosts.area?[neighIndex] == false) {   //Allowed area path cost switched from adding a constant to multiplying the path cost
